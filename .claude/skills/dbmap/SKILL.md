...
### Enable (`dbmap-auto-on`)
Write the DSN to `.dbmap-auto` so DBMAP.md regenerates after every migration command:
```bash
printf '%s\n' '<confirmed_dsn>' > .dbmap-auto
touch .gitignore && grep -qxF '.dbmap-auto' .gitignore || printf '\n.dbmap-auto\n' >> .gitignore
```
Requires DBMAP.md to exist already (run dbmap first). The hook fires on migration commands (Rails db:migrate, Django migrate, Alembic, Prisma, Knex, etc).

### Disable (`dbmap-auto-off`)
```bash
rm -f .dbmap-auto
```
DBMAP.md is kept as-is, can still be manually updated.
