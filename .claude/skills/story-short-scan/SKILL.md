...
| 优先级 | 模式 | 说明 | 何时用 |
|--------|------|------|--------|
| 0 | **MCP 实时数据** | 通过 hotnews-mcp + zhihu-mcp 获取知乎热榜/盐选排行 | MCP Server 已安装时（优先） |
| 1 | **browser-cdp 采集** | 直接抓取平台页面，产出结构化文件 | 无 MCP 但有 Chrome 环境时 |
| 2 | **用户提供** | 用户粘贴榜单截图/文字/链接 | 用户已有数据时 |
| 3 | **内置知识** | 基于知识库中的趋势数据和方法论做分析 | 无法联网、用户无数据时 |

**MCP 优于 CDP**：MCP 直接调用知乎 API，速度更快、数据更结构化、无需浏览器环境。MCP 不可用时自动降级到 CDP 或用户提供。

#### MCP 采集模式

当 hotnews-mcp 和 zhihu-search-mcp 可用时，直接通过 MCP 获取结构化数据：

```
mcp__hotnews__get_zhihu_hot → 实时热榜 (排名/标题/热度/标签)
mcp__zhihu-search__search → 盐选故事搜索 + 内容获取
```

MCP 数据直接注入 Phase 2 的题材分析，无需中间文件。
